                ...
                return upstream.flatMap(new Function<T, Publisher<T>>() {
                    ...
                    public Publisher<T> apply(T model) throws Exception {
                        if (model == null || model.isNull()) {
                            return Flowable.error(new NetException(model.getErrorMsg(), NetException.NoDataError, model.getCode()));
                        } else if (model.isAuthError()) {
                            return Flowable.error(new NetException(model.getErrorMsg(), NetException.AuthError, model.getCode()));
                        } else if (model.isBizError()) {
                            return Flowable.error(new NetException(model.getErrorMsg(), NetException.BusinessError, model.getCode()));
                        } else {
